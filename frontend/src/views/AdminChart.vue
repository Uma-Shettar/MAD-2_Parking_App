    ...
    <div class="row m-5">
        <div class="col-md-6">
            <div class="card border-primary">
                <div class="card-body">
                    <h5 class="card-title text-center">Revenue by Parking Lot</h5>
...
const lotRevenueChartData = computed(() => {
    return {
        labels: adminchartdata.value.lot_revenue_data.map(item => item.label),
        datasets: [
            {
                label: 'Revenue',
                backgroundColor: 'rgba(75, 192, 192, 0.2)',
                borderColor: 'rgba(75, 192, 192, 1)',
                data: adminchartdata.value.lot_revenue_data.map(item => item.value),
            },
        ],
    };
});
...
const lotReservationsChartData = computed(() => {
    return {
        labels: adminchartdata.value.lot_reservations_data.map(item => item.label),
        datasets: [
            {
                label: 'Reservations',
                backgroundColor: 'rgba(75, 192, 192, 0.2)',
                borderColor: 'rgba(75, 192, 192, 1)',
                data: adminchartdata.value.lot_reservations_data.map(item => item.value),
            },
        ],
    };
});
...
const fetchdata = ( async () => {
    ...
    try {
        const response = await fetch('http://127.0.0.1:5000/api/adminchart', {
            method: 'GET',
            mode : 'cors',
            headers: {
        ...
        adminchartdata.value.lot_revenue_data = data.lot_revenue_data || [];
        adminchartdata.value.lot_reservations_data = data.lot_reservations_data || [];
        console.log(adminchartdata.value);
    } catch (error) {
